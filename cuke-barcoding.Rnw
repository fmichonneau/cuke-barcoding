...
included in the analysis (Table~\ref{tab:isolation-by-distance-table}).  The
...
The sea cucumber diversity is underestimated. Regardless of the method used to
estimate ESUs from the genetic data, we recovered more species than we could
identify based on accepted taxonomic names. The manual assessment of the ESUs
...
distances higher than the distance to their most closely related species (10 out
of 15 ESUs), or ESUs that are not monophyletic. Interestingly the 5 ESUs
concerned all belong to the subgenus \textit{Halodeima}: 3 ESUs of
\textit{Holothuria edulis}, \textit{H. mexicana} and \textit{H. floridana}. The
species involved are all unambiguously morphologically different and could be
assigned to the correct species if a voucher specimen is
available. Additionally, we only detected a single case of putative
...
intermediate phenotype of its parent species.
...
that deserve to be investigate further.
...
latest as early as \textit{ca.} 19,000 years ago \cite{Siddal2003}. Therefore,
the Red Sea fauna must have colonized the area from outside refugia following
the salinity crisis. Of the 30 ESUs occurring in the Red Sea, five occur
throughout the Indo-Pacific suggesting that they can expend their ranges
rapidly.
...
often more than 10 millions years \cite{Meyer2005, Quenouille2011, Malay2010,
  Hodges2013}. The rapid acquisition of reproductive isolation could arise from
