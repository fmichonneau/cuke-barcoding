...
- largest barcoding dataset ever assembled
...
* How much diversity?
  ...
  sample
  - implications for conservation, fisheries management
  - methods that can be used:
    - limitations: some methods out of the question because size of dataset
    (e.g., GMYC)
    - use of thresholds: problem such large datasets, differences in rates of
    molecular evolution making single threshold approach potentially challenging
    - introduce pairwise and cluster methods challenges with both
      - show image that represent: differences in topologies
      (paraphyly/polyphyly)
      - long branches
    - does it really matter that ESUs identified aren't really species? they are
...
* Geography, diversity and diversification
  - Diversity:
...
\subsection{ESU delineation}

\paragraph{The local barcoding gap method} The local barcoding gap clustering
method (LBGM) is introduced here. It identifies monophyletic clusters of
individuals from a neighbor-joining tree (other tree reconstruction methods can
be used). Individuals are clustered so that the maximum genetic distance between
two individuals belonging to the same cluster is less than a set
threshold. Since it is based on the maximum intraspecific distance, it shares
similarity with the local barcoding graphs, where maximum intra-specific
