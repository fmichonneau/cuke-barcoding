...
complete the analysis (e.g. PTP \cite{Zhang2013}, or AGBD
\cite{Puillandre2012a}). Therefore, we were here limited to use threshold-based
...
the threshold identifies the barcoding gap by going from overspliting. We
repeated these analyses on the entire dataset, on each order and each family.

It has been suggested to use a threshold equals to 10 times the intraspecific
distance \cite{Hebert2004a}. However, this approach was not suitable in our
study as the high levels of cryptic species were considerably inflating the
...
% - use different colors to code species mostly in the Pacific, mostly in IO or
...
distributions. To obtain a diversity contour map, we superimposed the polygons
obtained.
...
estimated from the K2P distances.  If the distributions of the ESUs could be
inferred with a polygon, we classified their distributions as: parapatric if
less than 10\% of the range of the ESU with the smallest range overlapped with
the species with the larger range, sympatric is their ranges overlapped by more
than 10\%, and allopatric otherwise. If only one species had enough data to
estimate its distribution with a polygon, and points were available for the
other species, we considered their distribution allopatric if none of the points
were included in the polygon, and sympatric if they were.
...
tested provided similar estimates of the proportion of singletons.
...
Threshold insufficient as lumps ESUs that seem different.

Examples of possible oversplitting.
...
\caption{Overlay of the polygons representing the distributions of all ESUs
  delineated with the clustering method with a threshold of 3\%, occurring
...
relatively consistent across the Indo-Pacific with about at least 20 species
...
<<geography-diversification, cache=TRUE, echo=FALSE, fig.width=5, fig.height=3, fig.scap='Geography of diversification', fig.cap='Number of geographic types for ESU pairs identified by the clustering method with a 3\\% threshold'>>=
