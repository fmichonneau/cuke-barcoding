...
For the rest of the study we chose to use the ESUs as delineated by the
clustering method with a threshold of 3\% based on the K2P distances. This
threshold provided a compromise by delineating ESUs with different color
patterns or ecology, without splitting ESUs that showed some level of geographic
structure across their ranges.

With this threshold, the number of estimated ESUs is
\Sexpr{length(load_species_clusterGrps("K80", "all", 0.015))}. The difference
...
Our large scale sampling effort also led to the discovery of many species, many
