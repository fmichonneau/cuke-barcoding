...
\Sexpr{formatC(percentHIPO, width=2, digits=2)}. The levels of genetic
divergence for the ESU pairs separated across these basins varied greatly
(Fig.~\ref{fig:barriers}). For the Red Sea/Pacific Ocean (RS/PO), and for the
Indian Ocean/Pacific Ocean (IO/PO) pairs, distances clustered in two groups
(RS/PO: ESU pairs with genetic distances ~ 5\% and > 10\%; IO/PO: ESU pairs with
genetic distances < 5\% and > 7.5\%).
...
<<compare-manual-cluster-ESUs-plot, cache=TRUE, results='hide', fig.height=6.5, echo=FALSE, dependson='compare-manual-cluster-ESUs-data', fig.cap='Number of ESUs incorrectly assigned compared to the manual ESU delineation on the Holothuriidae for the thresholds using the pairwise distance method (pairwise) and theclustering method (cluster), based on uncorrected genetic distances or the Kimura 2-parameter distances (K2P).ESUs can be incorrectly assigned because of oversplitting (1 manually defined ESU results in several threshold-based ESU) or lumping (1 threshold-based ESU is comprised of several manually defined ESU). The lowest error rate is with the clustering method based on uncorrected distances.'>>=
