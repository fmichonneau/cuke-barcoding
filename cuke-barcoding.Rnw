...
compromise by separating ESUs with different color patterns or ecology, without
splitting ESUs that showed some level of geographic structure across their
range. This distance threshold also corresponds to the level where all methods
tested provided similar estimates of the proportion of singletons. We also
favored this approach as it led to reciprocally monophyletic groups that
facilitated the investigation of geographic patterns of diversity and
diversification. With this threshold, 16 of the 17 pairs of trans-isthmian
species were recovered as distinct ESUs. Only \textit{Euapta godeffroyi} and
\textit{E. lappa} were lumped in a single ESU, as they are characterized by the
lowest levels of intergenetic distances across the isthmus (mean
distance=1.95\%).
...
clearly differentiated morphologically, are potentially new species not directly
...
Aspidochirotida and Dendrochirotida (Fig.~\ref{fig:isolation-by-distance-plot},
Table~\ref{tab:ibd-stats}). Typically, ESUs that spanned the entire range of the
Indo-Pacific had higher genetic distances than species with restricted
ranges. However, there was a considerable amount of variation and a number of
...
(\Sexpr{formatC(percentSymp, digits=2, width=2)}\%), and or parapatric
(\Sexpr{formatC(percentSymp, digits=2, width=2)},
...
<<percent-endemism, dependson='global-diversity-map', cache=TRUE, echo=FALSE>>=
@

The highest diversity in sea cucumbers is not the coral triangle but the Western
Pacific. This area is where the distributions of species with ranges that span
most of the Indo-Pacific, species that span the Indian Ocean and extends in the
Western Pacific, and species that mostly occur in the Pacific overlap. The
polygon approach used to identify areas of high diversity has limitations. In
particular, it may overestimate diversity where species do not occur but occur
at the periphery. Some species included in our analysis have restricted ranges
or are known from a single specimen, and no polygons were drawn for
them. Therefore the diversity from the polygons is underestimated. For instance,
65 species have been recorded for Guam \cite{Michonneau2013}, but
...
but bindin is known from sea urchins [88] and sea stars [91] which indicates
that it is plesiomorphic in the clade that sea cucumbers emerged from.  They
could explain how species in the “H. impatiens” complex acquired reproductive
isolation quickly.


Rapid secondary sympatry with GRPs.
