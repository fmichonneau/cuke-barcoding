...
distances \cite{Meyer2005}. The characterization of this gap, or its absence,
requires a comprehensive sampling both within and across species.  If species
are sampled throughout their ranges, we can assess levels of intra-specific
variation, and with near-complete taxonomic sampling, it becomes possible to
determine how often an unidentified sequence can be assigned to a known species
accurately, but also to narrow down the identity of this unknown sequence to the
species with intraspecific distances incompatible with the barcoding
gap. Additionally, comprehensive sampling allows to gain insights into the
amount of cryptic diversity, and the patterns of diversification characterizing
the groups. It becomes possible to assess the generality of patterns unraveled
...
occurred in allopatry (e.g., \cite{Meyer2005-evolution, Volger2008, Malay2010})
...
<<barriers, cache=TRUE, dependson='geography-diversification', echo=FALSE, fig.height=3, fig.cap='Mean genetic distances between sister ESUs separated by the Indian Ocean (IO) and the Pacific Ocean (PO), the Red Sea (RS) and the Pacific Ocean, and Hawaii (HI) and the Pacific Ocean'>>=
...
\cite{Gaithier2010}. Here we found, xx\% of the species to show differences...
