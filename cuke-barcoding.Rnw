...
If speciation occurs in allopatry, recently diverged species may retain the
geographic signal of this event in their distributions. To test this hypothesis,
we compared the geographical ranges of sister ESUs. If both ESUs were sampled
enough so that a polygon approximating their distribution could be inferred, we
classified their distributions as: parapatric if less than 10\% of the range of
the species with the smallest range was included in the range of the species
with the larger range, sympatric is their ranges overlapped by more than 10\%,
and allopatric otherwise. If only one species had enough data to estimate its
distribution with a polygon, and points were available for the other species, we
considered their distribution allopatric if the points were not included in the
polygon, and sympatric if they were.
...
of sister ESUs. Most had allopatric distributions
(Fig.~\ref{fig:geography-diversification}). ESUs with sympatric distributions
were
