...
barcoding gap. Additionally, comprehensive sampling allows to gain insights into
the amount of cryptic diversity, and the patterns of diversification
characterizing the groups. It becomes possible to assess the generality of
patterns unraveled in species-specific studies.
...
understanding, and a pressing need to understand species limits, make sea
cucumbers a suitable group for a large-scale barcoding study.

When the lineages identified with genetic data are associated with clear
differences in morphology or other independent traits (behavior, ecology), the
concept of evolutionary significant unit (ESU) \cite{Moritz1994} applies, and
the lineages can be treated as species-level units. The discordance between
genetic data and morphological traits results from (1) recently diverged species
for which the mitochondrial lineages have not fully sorted according to the
species tree; (2) introgression of a mitochondrial lineage from another species;
(3) morphological polymorphism that does not reflect species limits
\cite{Funk2003, Ballard2004}. These three processes will lead to similar
patterns in mitochondrial genealogies with species appearing para- or
polyphyletic. If these cases might have undesirable effects on the accuracy of
species identification, they might provide insights into the biology of the
species. For instance, sympatric paraphyletic species might be indicative of the
rapid acquisition of reproductive isolation, while polyphyletic species might
provide indication of hybridization. Through comprehensive sampling, we can
identify species that show this pattern and deserve further investigation, and
assess how common they are.
...
geographical distance, may be detected. Larvae can be planktotrophic (they feed
in the plankton), or oligotrophic (they feed on reserves included with the
...
larvae. Therefore, we hypothesize that (1) species with oligotrophic development
(Dendrichoritda) will show a stronger IBD pattern, and (2) will have smaller
...
diversification in marine organisms. In most cases, cryptic lineages identified
occurred in allopatry (e.g., \cite{Meyer2005-evolution, Vogler2008, Malay2010})
but in sea cucumbers (see Chapter~2) recently diverged species do occur in
sympatry. How common is this pattern?
