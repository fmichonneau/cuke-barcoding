...
% introduce idea of barcoding gap
...
<<sampling-maps, cache=TRUE, echo=FALSE, warning=FALSE, fig.height=6, fig.cap='Locations sampled in this study'>>=
...
manufacturer's protocol were used.
...
We kept all sequences that were more than 600 bp and that did not include any
stop or non-sense codons that may have resulted from incorrect base calling
during the sequencing or the editing process.
...
threshold set \textit{a priori}. Since it is based on the maximum intraspecific
distance, it shares similarity with the local barcoding graphs, where maximum
intra-specific distance between individuals is plotted against the distance to
the closest neighboring species (see \cite[Fig.~4 in]{Robinson2009} for an
example). It differs as it only uses information from the tree (no need to
associate species names with the sequences), but suffers from the caveats
associated with using arbitrary thresholds.
...
from the barcoding data. For each ESU, when three or more locations were
sampled, we computed the convex hull to obtain a polygon approximating their
geographic distributions. We superimposed these polygons to obtain a diversity
contour map.

\subsubsection{Isolation-by-distance}

<<isolation-by-distance-data, cache=TRUE, results='hide', echo=FALSE, warning=FALSE>>=
@
...
we performed an analysis of covariance (ANCOVA) on the maximum genetic distance
for each species as the response variable, the maximum geographic distance as
the covariate, and the taxonomic order as the independent variable. We performed
the analysis on \Sexpr{paste(orderToInclude, collapse=', ')}.
The variances among the orders were homogeneous (Levene's test,
\Sexpr{leveneIbdFstat}, \Sexpr{leveneIbdP}).
...
recently diverged ESUs, we tested whether sister ESUs were sympatric or
allopatric. We identified sister ESUs based on the neighbor-joining tree
estimated from the uncorrected distances. If the distributions of the ESUs
could be inferred with a polygon, we classified their distributions as:
parapatric if less than 10\% of the range of the ESU with the smallest range
overlapped with the species with the larger range, sympatric is their ranges
overlapped by more than 10\%, and allopatric otherwise. If only one species had
enough data to estimate its distribution with a polygon, and points were
available for the other species, we considered their distribution allopatric if
none of the points were included in the polygon, and sympatric if they were.
...
<<groups-comparison-plot, dependson=c('pairwise-groups-data', 'cluster-groups-data', 'n-species'), cache=TRUE, echo=FALSE, fig.height=6.5, fig.cap='Comparison of the number of ESUs (top), and the proportion of ESUs represented by a single individual (singletons, bottom) estimated with the pairwise distance method (pairwise) and the clustering method (cluster), based on uncorrected distances (uncorrected) or the Kimura 2-parameter distances (K2P), for the entire dataset (all), the order Aspidochirotida and the family Holothuriidae. The gray horizontal dashed line represents the number of named species sequenced.', fig.scap='Estimated number of ESUs'>>=
...
exhaustively sampled (\Sexpr{formatC(100*nSppAsp/nSppAll, width=2, format="d")}
and \Sexpr{formatC(100*nSppHol/nSppAll, width=2, format="d")} of the total number
of individuals respectively), with all genera and subgenera of the Holothuriidae
represented, and 65\% for the currently accepted species names.
...
suggesting cryptic diversity . The pairwise distance method led to lower
...
4\% with the clustering method and for thresholds > 2\% with the pairwise
distance method.
...
For the rest of the study we use the ESUs as delineated by the clustering method
with a threshold of 4\% based on the raw distances. This threshold provided a
compromise by delineating ESUs with different color patterns or ecology, without
splitting ESUs that showed some level of geographic structure across their
range. This distance threshold also corresponds at the level where all methods
tested provided similar estimates of the proportion of singletons. We also
favored this approach as it led to reciprocally monophyletic groups that could
then easily be used to investigate geographic patterns of diversity and
diversification.

With this threshold, the number of estimated ESUs is
\Sexpr{length(load_species_clusterGrps("raw", "all", 0.02))}. The difference
with the number of named species comes from species complexes. These complexes
can be deeply divergent populations across the range of species previously
thought to be circum-tropical. For instance, \textit{Holothuria arenicola} forms
a complex with different ESUs in the Caribbean basin, the Eastern Pacific and
the rest of the Indo-Pacific. For this species and others (e.g.,
\textit{Holothuria hilla}), these divergent lineages have overlapping ranges,
which made their recognition as distinct evolutionary units
challenging. Additionally, many species thought to occur in both Indian and
Pacific Oceans (e.g., \textit{Holothuria pervicax}, \textit{Holothuria
  fuscocinerea}), or to reach peripheral areas of the Indo-Pacific (e.g.,
\textit{Holothuria cinerascens} has ESUs both in Hawaii and the Red Sea), appear
highly divergent genetically, sometimes accompanied with morphological
differences.

Our large scale sampling effort also led to the discovery of ESUs that appear to
be undescribed species. These include large, commercially important species. For
the Holothuriidae that have been the most comprehensively sampled, 12 ESUs
clearly differentiated morphologically, are potentially new species.

For some groups that were sampled extensively, we recovered less ESUs than there
are described species. Notably, in \textit{Opheodesoma}, that currently include
11 species, we only recovered 4 ESUs as well as 2 probably undescribed species.

In a few cases, groups of individuals that appear morphologically differentiated
were grouped together in an ESU. This was in particular the case of
\textit{Holothuria edulis} which harbors two color morphs: gray  and fuschia;
and several species in \textit{Stichopus}. One the other hand, there were some
cases of species that were split into several ESUs but that did not seem to be
differentiated morphologically and that had broadly overlapping ranges.


% Typically, the recommended threshold in barcoding studies is 10 times the
% average intraspecific distance \cite{Hebert2004a} when using the pairwise
% method. The intraspecific distance for the overall dataset was
% \Sexpr{formatC(mean(meanIntraSppAll, na.rm=TRUE)*100, digits=2)}\%
% (N=\Sexpr{formatC(sum(!is.na(meanIntraSppAll)))}),
% \Sexpr{formatC(mean(meanIntraSppAsp, na.rm=TRUE)*100, digits=2)}\% for the
% Aspidochirotida (N=\Sexpr{formatC(sum(!is.na(meanIntraSppAsp)))}),
% \Sexpr{formatC(mean(meanIntraSppDen, na.rm=TRUE)*100, digits=2)}\% for the
% Dendrochirotida (N=\Sexpr{formatC(sum(!is.na(meanIntraSppDen)))}),
% \Sexpr{formatC(mean(meanIntraSppApo, na.rm=TRUE)*100, digits=2)}\%
% (N=\Sexpr{formatC(sum(!is.na(meanIntraSppApo)))}) for the Apodida,
% \Sexpr{formatC(mean(meanIntraSppEla, na.rm=TRUE)*100, digits=2)}\%
% (N=\Sexpr{formatC(sum(!is.na(meanIntraSppEla)))}) for the Elasipodida,
% \Sexpr{formatC(mean(meanIntraSppMol, na.rm=TRUE)*100, digits=2)}\%
% (N=\Sexpr{formatC(sum(!is.na(meanIntraSppMol)))}) for the Molpadida. The 3\%
% thres
...
<<global-diversity-map, cache=TRUE, results='hide', echo=FALSE>>=
@

\begin{figure}
\begin{centering}
\includegraphics{figures/diversity-map}
\caption{Overlay of the polygons representing the distributions of all ESUs
  delineated with the clustering method with a threshold of 4\%, occurring
  between -30$^{\circ}$ and 30$^{\circ}$. The shade of red indicate the number
  of species co-occuring. The maximum number of overlapping ranges occurs
  North of Papua New Guinea with \Sexpr{sum(nSppPNG)} ESUs. }
\label{fig:diversity-map}
\end{centering}
\end{figure}


The highest diversity occurs in the Western Pacific with a maximum of
\Sexpr{sum(nSppPNG)} ESUs North East of New Guinea. Overall, diversity is
relatively consistent across the Indo-Pacific with about at least 25 ESUs
co-occurring from Madagascar to Hawaii, and from Taiwan to French
Polynesia. Diversity in the Indo-Malay archipelago is comparable to, or lower than
in other parts of the range with few ESUs restricted to this geographic area
(Philippines: \Sexpr{sum(nSppPhil)} ESUs, French Polynesia:
\Sexpr{sum(nSppPoly)} ESUs, Madagascar: \Sexpr{sum(nSppMada)} ESUs, Ryukyus:
\Sexpr{sum(nSppOki)} ESUs).


\subsubsection{Isolation-by-distance}

<<isolation-by-distance-plot, cache=TRUE, results='hide', echo=FALSE, warning=FALSE, dependson='isolation-by-distance-data', results='hide', fig.scap='Genetic and geographic distances for each order', fig.cap='Maximum genetic distance and maximum geographic distance for ESUs identified with the clustering approach with a threshold of 4\\%, represented by at least 3 individuals. Dashed lines represent the model estimated by an ANCOVA using the order as the independent variable.'>>=
@
...
The ANCOVA testing for the effect of maximum geographic distance and ``order''
...
distance (\Sexpr{interactionFstat}, \Sexpr{interactionP}), nor nor ``order''
term (\Sexpr{interceptFstat}) were significant, indicating a common slope and a
common intercept for the orders included in the analysis
(Table~\ref{tab:ibd-stats}).

We recovered a signal of isolation-by-distance (IBD) for Apodida,
Aspidochirotida and Dendrochirotida, but not for Elasipodida
(Fig.~\ref{fig:isolation-by-distance-plot},
Table~\ref{tab:ibd-stats}). Typically, ESUs that spanned the
entire range of the Indo-Pacific had higher genetic distances than species with
restricted ranges. However, there was a considerable amount of variation and a
number of species with large ranges were characterized by low genetic
differences. For instance, among the Dendrochirotida, \textit{Afrocucumis
...
For the tropical species, the means of the maximum distances for the Apodida
(\Sexpr{round(meanRange["Apodida"],2)}~km, SD= \Sexpr{round(sdRange["Apodida"],
  2)}) and the Aspidochirotida (\Sexpr{round(meanRange["Aspidochirotida"], 2)}~km,
SD=\Sexpr{round(sdRange["Aspidochirotida"], 2)}) were larger than for the
Dendrochirotida (\Sexpr{round(meanRange["Dendrochirotida"], 2)}~km,
SD=\Sexpr{round(sdRange["Dendrochirotida"], 2)}).
...
We compared the geographic ranges of \Sexpr{sum(complete.cases(spComp))} pairs
of sister ESUs. Most had allopatric distributions (\Sexpr{formatC(percentAllo,
  digits=2, width=2)}\%), the other sister ESUs were
(\Sexpr{formatC(percentSymp, digits=2, width=2)}\%) sympatric, and none were
parapatric (Fig.~\ref{fig:geography-diversification}).
...
\caption[Geographical barriers separating ESUs]{Approximate locations of the
  geographical barriers separating sister ESUs. Numbers indicate number of
  sister ESUs separated by the barrier. Pink lines indicate barriers separating
  3 sister ESUs. Dashed orange lines indicate barriers separating 1 sister ESU.}
...
As with all threshold-based method, in
particular, over such taxonomic and latitudinal breadth, this threshold led to
some level of type I and type II errors. Estimating them at this stage is
made challenging by high levels of unrecognized diversity.

Differences in rates of evolution

In some cases, less diversity than expected.

\subsection{Geography, diversity and diversification}

The polygon approach used to identify areas of high diversity has
limitations. In particular, it may overestimate diversity in areas where species
do not occur but occur at the periphery. Some species included in our analysis
have restricted ranges or are known from a single specimen, and no polygons were
drawn for them. Therefore the diversity from the polygons is underestimated. For
instance, 65 species have been recorded for Guam \cite{Michonneau2013}, but
\Sexpr{sum(nSppGuam)} ESUs are estimated from the polygons.
...
The absence of evidence of isolation-by-distance for the Elasipodida, might be
caused by the more limited geographical sampling for this species. Most of the
Elasipodida included in our study were collected around Antarctica, and the
maximum distance recorded was \Sexpr{maxRangeElasiF}~km. The analyses repeated
to only include species with range sizes similar to the Elasipodida,
recovered the evidence for isolation-by-distance for the Aspidochirotida and the
Dendrochirotida, but not for the Apodida. Additional sampling is needed to
determine whether the absence of relationship for the Elasipodida holds at
