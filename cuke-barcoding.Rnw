...
limits. In particular in groups where taxonomy is challenging, barcoding
unraveled cryptic lineages, most often associated with differences in
morphology, coloration or behavior, indicative of reproductive
...
the barcoding gap: intra-specific genetic distances cannot overlap with the
...
we might assess levels of intra-specific variation, and if most species of the
...
to gain insights into the amount of cryptic diversity in the group, and the
patterns of diversification of the group.

Here, we assembled one of the largest, and the most comprehensive
(geographically and taxonomically) barcoding data set targeting sea
cucumbers. This dataset allows us to investigate levels of cryptic diversity and
the geographic patterns of species diversity. Specifically, we ask (1) How does
the current sea cucumber taxonomy compare with the number of species estimated
with barcoding? (2) How often do species are characterized by a pattern of
genetic differentiation allowing identification with barcoding? (3) How does the
type of larva influence the geographical distribution and the levels of genetic
